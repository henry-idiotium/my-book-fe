 ...
 */
export function useBaseAxios<TGlobalResponse = unknown, TGlobalBody = unknown>() {
  const navigate = useNavigate();
  const location = useLocation();

  const [authData, refresh] = useAuthGuard();

  const { token, expires, refreshUninitialized, isRefreshing, refreshError } = authData;
  ...
  async function executeRequestWithAuthGuard<TRes, TBody>(config: UseAxiosRequestConfig<TBody>) {
  ...
  function executeAuthAwareRequest<TRes, TBody>(
    ...
    const execute = options.useAuth ? executeRequestWithAuthGuard : executeRequest;
